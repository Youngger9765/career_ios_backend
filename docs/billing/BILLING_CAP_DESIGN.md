...
| 設定 | 金額 |
|------|------|
| **Gemini 建議上限** | **$800 USD**（留 50% buffer） |
| 絕對硬上限 | $1,200 |

> **Gemini 設定：月度 $800 USD 上限**（超過發 alert，不自動斷）
...
| **Gemini (Vertex AI)** | $540 | **$800** | $1,200 |
| Cloud Run | $100 | $200 | $300 |
| Supabase | $25 | $25 | $25（固定） |
| OpenAI Embedding | $2 | $10 | $20 |
| **總計** | **$2,267** | **$3,035** | **$4,045** |
| | | | |
| 月營收 | | $6,250 | |
| **毛利** | **$3,983** | **$3,215** | **$2,205** |
| **毛利率** | **64%** | **51%** | **35%** |
...
| 規則 | 建議值 | 說明 |
|------|--------|------|
| 月度使用時長 | **3 小時 (180 credits)** | 100 TWD 方案，43% 毛利 |
| 單日使用上限 | 3 小時 | 異常偵測，超過發 alert |
| 單次 session 上限 | 60 分鐘 | 防止忘關 |

### 8.5 100 TWD 方案時長 vs 毛利率
...
> **6 小時會虧 13%，若要給 6 小時，月費需漲到 190 TWD。**

### 8.6 GCP Billing Alert 設定
...
| **100 TWD 方案** | **180 credits = 3 小時/月**（毛利 43%） |
| **單用戶日上限** | 3 小時（異常偵測） |
| **單次 session 上限** | 60 分鐘 |
...
> **一句話總結：ElevenLabs 設 $2,000/月，Gemini 設 $800/月，單用戶 3 小時/月（180 credits = 100 TWD）。總預算 $3,000/月，保持 50%+ 毛利率。**

---

## 參考文件

- `COST_ANALYSIS.md` — 成本分析 v3.0（根目錄）
