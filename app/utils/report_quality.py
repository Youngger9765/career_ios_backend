...
def generate_quality_summary(
    ...
    critical_sections_cited_count = sum(
        1 for r in citation_validation["section_details"].values() if r["has_citations"]
    )

    return {
        "structure_quality": {
            "coverage": structure_validation[
                "coverage"
            ],  # Frontend expects "coverage" not "completeness"
            "completeness": structure_validation[
                "coverage"
            ],  # Keep for backward compatibility
            "missing_sections": structure_validation["missing_sections"],
            "status": "✅ 完整"
            if structure_validation["complete"]
            else f"❌ 缺少 {len(structure_validation['missing_sections'])} 個段落",
        },
        "citation_quality": {
            ...
            "all_critical_sections_cited": citation_validation[
                "all_critical_sections_cited"
            ],  # Keep boolean for logic
...
async def generate_quality_summary_with_llm(
    ...
    llm_grade = await grade_report_with_llm(
        report_text=report_text, use_legacy=use_legacy, client=openai_client
    )
