...
class EmailSenderService:
    ...
    def _generate_subject(self, report_data: Dict[str, Any]) -> str:
        ...
        report_date = datetime.fromisoformat(report_data["report_date"]).strftime("%Y-%m-%d")
...
"""
    ...
    def _build_services_table(self, top_services: List[Dict]) -> str:
        ...
        for i, svc in enumerate(top_services, 1):
            ...
            changes = [d.get("change_pct") for d in svc["data_points"] if d.get("change_pct") is not None]
            avg_change = sum(changes) / len(changes) if changes else 0
            change_class = "positive" if avg_change < 0 else "negative" if avg_change > 0 else ""
