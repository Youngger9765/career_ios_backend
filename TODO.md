...
### Phase 2: 🟡 API 架構統一 (下週，12-18h)
3. **P0-B: Mode 支援** (6-8h) ← ✅ **已完成** (2025-12-31)
4. **P0-C: API 統一** (12-18h) ← 🟡 **Phase 1 完成** (2025-12-31)，Phase 3/4 暫緩

### Phase 3: 🟢 體驗優化 (第 3 週，6-8h) ← 🎯 **當前優先**
5. **P1-B: Streaming 支援** (6-8h) ← 感知延遲從 5.61s → 1-2s，**下一個任務**

### Phase 4: 🔵 產品增強 (第 4 週+，10-12h)
6. **P2: Prompt 升級 - 8 大流派** (10-12h) ← 產品功能擴充

**關鍵決策邏輯** (更新 2025-12-31):
- ✅ 先修 Bug (RAG 失效) → 再清架構 (配置) → 再統一 API (部分完成) → **現在優化體驗**
- 🎯 P0-C Phase 1 完成後，P1-B Streaming 優先於 P0-C Phase 3/4（用戶體驗改善更直接）
...
## ✅ P0-B: analyze-partial API mode 支援 (已完成 2025-12-31)
...
- [x] **Service 層** (app/services/keyword_analysis_service.py): ✅
  ...
  - 儲存時 `result_data["_metadata"]["mode"] = mode.value` ✅
...
### 達成成果 ✅
- ✅ analyze-partial API 完整支援 emergency/practice 模式
- ✅ realtime.py bug 修復（analysis_type 和 mode 正確分離）
- ✅ 4 個整合測試覆蓋所有情境
- ✅ 向後相容（mode 參數可選，預設 practice）
- ✅ 完整文檔更新（PRD, CHANGELOG, IOS_API_GUIDE）
- ✅ 0 breaking changes
...
**優先級**: 🔴 P0 (API 統一的前置需求)
**實際時間**: 已包含在 commit 21f8137 (實作) + commit 3aeeb49 (文檔)
**Git Commits**: 21f8137 (implementation), 3aeeb49 (documentation)
**風險**: 低

---

## 🟡 P0-C: API 統一 - 淘汰 realtime.py 分析邏輯 (部分完成，Phase 3/4 延後)

### ✅ Phase 1 完成狀態 (2025-12-31)

**已完成**:
- ✅ Sliding Window prompt 註解整合（commit b2ad98f）
- ✅ 所有租戶 prompt 品質提升至 realtime.py 同等級
- ✅ analyze-partial 功能完整性達成（RAG + Mode + Sliding Window + 高品質 Prompt）
- ✅ 373 個整合測試全通過，無回歸問題

**關鍵決策** (2025-12-31):
- 📋 **Phase 3/4 (Web Console 遷移 + 棄用 realtime.py) 暫緩執行**
- 🎯 **優先轉向 P1-B Streaming 支援**（用戶體驗改善更直接）
- 💡 **理由**: analyze-partial 已功能完整，兩套 API 並存維護成本可接受
...
**實際情況更新** (2025-12-31):
- ✅ analyze-partial 已完整（功能對等 realtime.py）
- ✅ iOS App 已完全使用 analyze-partial（681 行測試通過）
- 🟡 Web Console 仍使用 realtime.py（運作正常，無緊急問題）
- 📊 維護成本實際上已降低（keyword_analysis_service 很完善）

### 架構目標（原始）

```
當前 (2025-12-31 實際狀態):
Web Console → realtime.py /analyze → 舊分析邏輯（運作正常）
iOS App     → analyze-partial API  → 新分析邏輯（功能完整 ✅）
...
|------|-------------|----------------|------|
| **Sliding Window** | ✅ 有 (10 turns) | ✅ 已完成 (2025-12-31) | ✅ Prompt 註解已整合 |
| **Cache Management** | ✅ 有 | ❌ 無 | 不需要 (實測效果差) |
| **Multi-Tenant** | ❌ 無 | ✅ 有 | 保留 analyze-partial |
| **RAG 整合** | ❌ 無 | ✅ 有 (已修復) | P0-A 完成 |
| **Mode 支援** | ✅ 有 (emergency/practice) | ✅ 已完成 (2025-12-31) | P0-B 完成 |
| **Prompt 品質** | ✅ 高 (詳細) | ✅ 已同步 (2025-12-31) | 合併完成 |
...
### Phase 3: 實作任務清單 (8-12h) - ⏸️ 暫緩執行

**決策** (2025-12-31):
- 🛑 **Task 1-4 全部暫緩**，不執行 unified_analysis_service 創建
- ✅ **保持現狀**: keyword_analysis_service 已功能完整，無需統一
- 🎯 **優先轉向**: P1-B Streaming 支援（用戶體驗改善更直接）

**何時重啟 Phase 3/4**:
- ⚠️ 如果 realtime.py 出現 bug 需修復（但 analyze-partial 沒有）
- ⚠️ 如果兩邊邏輯開始分歧造成維護問題
- ⚠️ 如果產品決策要淘汰 Web Console
- 📊 否則維持現狀（維護成本可接受）
