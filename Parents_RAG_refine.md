# Parents Realtime 分析系統改進計劃

## 文件目的

記錄 Realtime 親子對話分析系統的問題診斷、改進方案和實作進度。
遵循 **PDCA（Plan-Do-Check-Act）** 循環，持續優化系統效果。

---

## 📋 Plan（現況分析與改進計劃）

### 問題定義

**來源**：12/18 晚上 nini 老師使用 APP 後的反饋
> "老師回饋他給的建議基本上用不太到，與實際情況有差異"

**根本原因分析**（app/api/realtime.py）：

1. **分析範圍太窄**（最關鍵）
   - Line 49-52: 只分析「最新一分鐘」對話
   - 忽略重要上下文，導致建議脫離實際情況

2. **Practice 模式建議太長**
   - Line 67-68: `< 50 字`
   - 應該：`≤ 15 字`（像專家建議一樣簡短）

3. **LLM 自己生成建議**
   - 不穩定、太長、不夠專業
   - 應該：從 200 句專家建議中選擇

4. **缺少 Bridge 技巧**
   - 現有系統偏向「評價」
   - 缺少「連結」和「穩定」機制

5. **風險評估太粗糙**
   - Line 294-345: 只用關鍵字匹配
   - "你讓我受不了" vs "這個遊戲讓我受不了" 都誤判為 RED

6. **RAG 檢索不足**
   - Line 688-699: top_k=3, 內容截斷 200 字
   - 檢索到的知識太少

7. **角色混淆**
   - 系統假設：counselor → parent (client)
   - 12/18 實際：teacher → child
   - 需要角色自適應

---

### 核心改進方向

#### 1. 專家建議系統（200 句即時回饋）

親子溝通專家提供的 **200 句即時回饋建議**，用三色燈號分類：

##### 🟢 綠色｜對話安全（70 句）
**目的**：強化正向行為，說明有效溝通的關鍵

**核心原則**：
- 接住情緒：先理解感受，不急著解決問題
- 建立信任：讓孩子知道你站在他這邊
- 尊重理解：把孩子當成一個人，不是問題

**典型建議句**（每句 ≤ 15 字）：
```
- 讓孩子知道你站在他這邊 (12字)
- 你先理解感受，而不是處理事情 (14字)
- 你的語氣讓孩子感到被尊重 (12字)
- 你沒有急著給答案 (8字)
- 你在陪，而不是推 (7字)
```

##### 🟡 黃色｜需要調整（65 句）
**目的**：指出可改善點，提供替代說法

**警訊指標**：
- 開始說理：太快進入解釋、講道理
- 情緒失焦：孩子的感受還沒被接住
- 站到對立：開始評價、要求、比較

**典型建議句**：
```
- 這句話可能讓孩子開始防衛 (13字)
- 你有點太快進入說理 (9字)
- 孩子的感受還沒被接住 (10字)
- 你開始在解釋自己了 (9字)
```

##### 🔴 紅色｜立刻修正（65 句）
**目的**：警告危害，提供緊急修正方案

**危險行為**：
- 直接傷害：貼標籤、威脅、羞辱
- 情緒暴力：情緒勒索、冷暴力
- 破壞關係：否定存在、切斷連結

**典型建議句**：
```
- 這句話會直接傷到孩子 (10字)
- 你正在用指責的方式說話 (11字)
- 這是威脅性的語氣 (8字)
- 請立刻停下來 (6字)
```

---

#### 2. Bridge 技巧（焦點解決取向）

**來源**：Solomon Chen 專家諮詢（2025-12-25 23:32）

**核心理念**：
> 用鼓勵和連結的話語，把家長從「我被問題困住了」帶到「我曾經成功過，我可以做些什麼」

**三步驟架構**：

1. **讚美（Compliments）**
   - 肯定過去努力和做得好的地方
   - 範例：「你願意反思自己的行為，這已經是很大的進步了」

2. **橋樑（Bridge）**
   - 用提問連結過去成功經驗
   - 範例：「你之前有沒有哪次成功壓住情緒的經驗？當時你做了什麼？」

3. **建議任務（Suggesting tasks）**
   - 提出具體的小任務
   - 範例：「下次遇到類似狀況，試著先深呼吸三次，再開口說話」

**應用到三色燈號**：

- **🟢 綠色**：讚美 → 橋樑 → 延伸
- **🟠 橘色**：肯定 → 提醒 → 替代
- **🔴 紅色**：穩住 → 同理 → 修正（最重要！）

**紅色燈號的三階段設計**：
```
1. 先穩住：「理解在教養壓力下，情緒失控是很正常的」
2. 降低防衛：「你一定也很辛苦，想要孩子好但不知道怎麼做」
3. 再提醒：「這句話可能會讓孩子感到害怕，我們可以試試這樣說...」
```

---

### 改進目標

**立即目標**（本週）：
- ✅ **Emergency 模式**：1-2 句（從 200 句專家建議中選）
- ✅ **Practice 模式**：3-4 句（從 200 句專家建議中選）
- ✅ 從 200 句專家建議中選擇，不讓 LLM 自己生成
- ✅ 加入 Bridge 技巧結構
- ✅ 前端改掉兩行格式判斷（統一單行格式）
- ✅ 建立 GBQ Table 並實作非同步寫入（Web 版暫不寫 DB）

**短期目標**（2 週內）：
- 擴大分析範圍（從 1 分鐘 → 3-5 分鐘）
- 改進風險評估邏輯（上下文理解，非關鍵字匹配）
- 增強 RAG 檢索（top_k=5-10, 移除內容截斷）
- **覆盤統整**：會談結束後，使用現有 **Report API** 生成總結報告（不需要新功能）

**不做**（過度工程化）：
- ❌ 向量化 200 句建議 → Chroma DB
- ❌ Looker Studio Dashboard（現階段）
- ❌ 複雜的 ETL Pipeline
- ❌ 複雜的 Re-ranking、Hybrid Retrieval
- ❌ A/B Testing 框架（現階段）
- ❌ 覆盤統整的額外新邏輯（用現有 Report API 即可）

---

## 🛠️ Do（實作方案）

### 方案 1：簡單優先 - 直接放 Prompt

**❌ 拒絕過度工程化**：
- 不建立獨立的 Chroma DB Collection
- 不做向量搜尋
- 不做複雜的 Hybrid Retrieval

**✅ 採用簡單方案**：

#### 方法 A（優先）：直接列在系統提示詞

```python
CACHE_SYSTEM_INSTRUCTION = """
你是親子溝通分析助手...

【專家建議句庫】請從以下建議中選擇 2-3 句最符合當前對話的：

🟢 綠色｜對話安全（選 1-2 句）：
- 讓孩子知道你站在他這邊
- 你先理解感受，而不是處理事情
- 你的語氣讓孩子感到被尊重
- 你沒有急著給答案
- 你在關心人，而不只是結果
... (全部 70 句)

🟠 橘色｜需要調整（選 1-2 句）：
- 這句話可能讓孩子開始防衛
- 你有點太快進入說理
- 孩子的感受還沒被接住
- 你開始在解釋自己了
... (全部 65 句)

🔴 紅色｜立刻修正（選 1-2 句）：
- 這句話會直接傷到孩子
- 你正在用指責的方式說話
- 這是威脅性的語氣
- 請立刻停下來
... (全部 65 句)

【Bridge 技巧】請按照以下結構組織回饋：

綠色：讚美 → 橋樑 → 延伸
橘色：肯定 → 提醒 → 替代
紅色：穩住 → 同理 → 修正

【分析步驟】：
1. 判斷對話安全等級（green/orange/red）
2. 從對應顏色的建議句中選出 2-3 句最符合的
3. 使用 Bridge 技巧組織回饋
4. 每句建議 ≤ 15 字
"""
```

**為什麼這樣就夠了？**
- ✅ 200 句建議總共約 3000 字，Token 成本極低
- ✅ LLM 理解力足夠，不需要向量檢索
- ✅ 實作簡單，維護容易
- ✅ 回應速度快（省去向量檢索時間）
- ✅ 可直接利用 Gemini Context Caching（7200s TTL）

#### 方法 B（備案）：如果 Prompt 超過 Token 限制

```python
# app/config/parenting_suggestions.py
GREEN_SUGGESTIONS = [
    "讓孩子知道你站在他這邊",
    "你先理解感受，而不是處理事情",
    # ... 70 句
]

ORANGE_SUGGESTIONS = [...]
RED_SUGGESTIONS = [...]

# 在 realtime.py 中動態組合
def build_prompt_with_suggestions(safety_level: str) -> str:
    if safety_level == "green":
        suggestions = GREEN_SUGGESTIONS
    elif safety_level == "orange":
        suggestions = ORANGE_SUGGESTIONS
    else:
        suggestions = RED_SUGGESTIONS

    suggestions_text = "\n- ".join(suggestions)
    return f"【{safety_level.upper()} 建議句庫】：\n- {suggestions_text}"
```

---

### 方案 2：資料持久化（PostgreSQL + GBQ）

#### 資料模型（現有 Schema）

**核心關係**：
```
Counselor → Client → Case → Session

一個 Counselor 有多個 Client
一個 Client 有多個 Case（不同諮商主題）
一個 Case 有多個 Session（會談記錄）
```

**親子對話的實作**（使用現有 Schema）：
- **Tenant**: "浮島親子"（已存在）
- **Counselor**: 家長（在浮島親子 tenant 下）
- **Client**: 孩子
- **Case**: 親子對話案例
- **Session**: 親子對話記錄
  - `session_type` 可用現有欄位區分（未來可擴展）
  - `analysis_logs` 存 Realtime 分析結果

**未來擴展**（暫不實作）：
- Schema 調整（counselor_type, client_type, session_type）
- 多租戶親子對話環境

#### 當前現狀

**✅ DB 結構已準備好**：
- `Session.analysis_logs`（JSON 欄位）已存在於 `app/models/session.py`

**❌ Realtime API 尚未實作寫入邏輯**：
- `app/api/realtime.py` 只做即時分析並回傳 JSON
- 沒有 `db.add()` 或 `.commit()` 程式碼

#### 實作計劃（簡單優先）

**✅ PostgreSQL - 必做**

```python
# Session.analysis_logs 欄位格式
{
  "analyzed_at": "2025-12-25T23:45:00Z",
  "analysis_type": "practice",             # emergency | practice
  "safety_level": "orange",                # green | orange | red
  "matched_suggestions": [
    "這句話可能讓孩子開始防衛",
    "你有點太快進入說理"
  ],
  "counselor_id": "uuid-xxx"
}
```

**用途**：
- 諮詢師查看歷史分析記錄
- 追蹤對話品質趨勢（綠→橘→紅）
- 與 Session/Case 關聯查詢

**✅ GBQ - 必做（簡單版）**

```sql
-- Table: realtime_analysis_logs
CREATE TABLE realtime_analysis_logs (
  id STRING,
  tenant_id STRING,
  session_id STRING,
  counselor_id STRING,
  analyzed_at TIMESTAMP,
  analysis_type STRING,              -- emergency | practice
  safety_level STRING,               -- green | orange | red
  matched_suggestions ARRAY<STRING>,
  transcript_segment STRING,
  response_time_ms INT64,            -- API 回應時間（毫秒）
  created_at TIMESTAMP
)
PARTITION BY DATE(analyzed_at)
CLUSTER BY tenant_id, safety_level;
```

**用途**：
- 跨租戶分析：哪些建議最有效？
- 燈號分佈統計：綠/橘/紅比例
- 長期趨勢分析：對話品質改善情況
- 系統效能監控：回應時間、使用量

**資料流**：
```
Realtime API 分析完成
    ↓
1. 回傳 JSON 給前端（即時）
    ↓
2. 寫入 PostgreSQL（同步，< 50ms）
    ↓
3. 背景任務：非同步寫入 GBQ
```

---

### 實作步驟

#### Phase 1（立即做 - Web 版本優先）

##### 1.1 建立 200 句建議檔案

- [x] **建立檔案**：`app/config/parenting_suggestions.py`
  - [x] GREEN_SUGGESTIONS = [...]  # 70 句
  - [x] YELLOW_SUGGESTIONS = [...]  # 65 句
  - [x] RED_SUGGESTIONS = [...]  # 65 句
  - [x] 格式：簡單的 Python list（不要 TypedDict）
  - [x] 驗證通過（200 句，無重複）

##### 1.2 修改後端 API（`app/api/realtime.py`）

- [x] **修改 Emergency Mode Prompt**
  - [x] 從 200 句專家建議中選 1-2 句
  - [x] 移除兩行格式（心法\n做法）
  - [x] 加入 Bridge 技巧結構（穩住 → 同理 → 修正）

- [x] **修改 Practice Mode Prompt**
  - [x] 從 200 句專家建議中選 3-4 句
  - [x] 加入 Bridge 技巧結構（讚美 → 橋樑 → 延伸）
  - [x] 加入 200 句建議到 Prompt（已內嵌）

- [x] **改進燈號判斷邏輯**
  - [x] LLM 判斷上下文（不只關鍵字匹配）
  - [x] 明確回傳 safety_level（green/yellow/red）
  - [x] **CRITICAL FIX**: 修正為紅黃綠交通號誌系統

- [x] **擴充 API 回傳格式**
  - [x] 新增 `"safety_level": "green|yellow|red"`
  - [x] 確保 suggestions 從 200 句中選擇
  - [x] 測試 Emergency 模式：1-2 句（✅ 9/9 tests passed）
  - [x] 測試 Practice 模式：3-4 句（✅ 9/9 tests passed）

**Phase 1.2 完成**（2025-12-25）：
- ✅ 修改 Emergency/Practice Mode Prompt（內嵌 200 句建議）
- ✅ 加入 Bridge 技巧結構（穩住→同理→修正 / 讚美→橋樑→延伸）
- ✅ API 新增 `safety_level` 欄位（green/yellow/red）
- ✅ LLM 從 200 句專家建議中選擇（不自己生成）
- ✅ **CRITICAL FIX**: 修正為紅黃綠交通號誌系統（orange → yellow）
- ✅ 建立整合測試（9 個測試全部通過）
- ✅ **所有測試通過（9/9 integration tests + 28/28 unit tests）**

##### 1.3 資料持久化（GBQ Only - Web 版暫不寫 DB）

**決策**：
- ✅ **GBQ 寫入**（必做）：記錄所有分析結果，用於長期分析
- ❌ **PostgreSQL 寫入**（Web 版暫不做）：Web 無 session 概念，等 iOS 版再實作
- 🏷️ **tenant_id 固定**：`"island_parents"`（浮島親子）

- [x] **建立 GBQ Table Schema**
  - [x] Table 名稱：`realtime_analysis_logs`
  - [x] 欄位設計：
    ```sql
    CREATE TABLE realtime_analysis_logs (
      id STRING,                          -- UUID
      tenant_id STRING,                   -- 固定 "island_parents"
      session_id STRING,                  -- Web 版為 null，iOS 版有值
      analyzed_at TIMESTAMP,              -- 分析時間
      analysis_type STRING,               -- "emergency" | "practice"
      safety_level STRING,                -- "green" | "yellow" | "red"
      matched_suggestions ARRAY<STRING>,  -- 選中的專家建議
      transcript_segment STRING,          -- 被分析的逐字稿片段
      response_time_ms INT64,             -- API 回應時間（毫秒）
      created_at TIMESTAMP                -- 記錄建立時間
    )
    PARTITION BY DATE(analyzed_at)
    CLUSTER BY tenant_id, safety_level;
    ```

- [x] **實作 GBQ 寫入（非同步）**
  - [x] 在 `realtime.py` 加入 GBQ 寫入邏輯
  - [x] 使用 FastAPI BackgroundTasks
  - [x] 資料格式：
    ```python
    {
      "id": str(uuid.uuid4()),
      "tenant_id": "island_parents",      # 固定
      "session_id": None,                 # Web 版為 None
      "analyzed_at": datetime.utcnow(),
      "analysis_type": "emergency|practice",
      "safety_level": "green|yellow|red",
      "matched_suggestions": [...],
      "transcript_segment": "...",
      "response_time_ms": 1500,
      "created_at": datetime.utcnow()
    }
    ```

- [x] **錯誤處理**
  - [x] GBQ 寫入失敗不影響 API 回應（catch exception）
  - [x] 記錄錯誤到 logging（不要 silent fail）
  - [x] 如果 GBQ unavailable，API 仍然正常回傳

- [x] **測試 GBQ 寫入**
  - [x] 測試資料成功寫入 GBQ
  - [x] 測試非同步執行不阻塞 API 回應
  - [x] 測試錯誤處理（GBQ 寫入失敗時）

**Phase 1.3 完成**（2025-12-25）：
- ✅ 建立 GBQ Service 模組（`app/services/gbq_service.py`）
- ✅ 實作 lazy client initialization（避免 CI 環境 auth 錯誤）
- ✅ 整合 FastAPI BackgroundTasks（非同步寫入）
- ✅ 實作 `write_to_gbq_async()` wrapper（錯誤處理）
- ✅ 修改 `analyze_transcript()` 加入 GBQ 資料準備和背景任務
- ✅ 建立 integration tests（TDD RED → GREEN）
- ✅ **所有測試通過（5/5 GBQ persistence tests）**
- ✅ tenant_id 固定為 "island_parents"（Web 版）
- ✅ session_id 固定為 None（Web 版無 session 概念）

##### 1.4 前端調整（`app/templates/realtime_counseling.html`）

- [ ] **移除兩行格式判斷**
  - [ ] 移除 Line 2533-2542 的 `if (sug.includes('\n'))` 判斷
  - [ ] 統一使用單行格式顯示

- [ ] **卡片視覺反應（根據燈號）**
  - [ ] 🟢 **綠燈卡片**：`bg-green-50 border-green-200 text-green-700`
  - [ ] 🟠 **黃燈卡片**：`bg-yellow-50 border-yellow-200 text-yellow-700`
  - [ ] 🔴 **紅燈卡片**：`bg-red-50 border-red-200 text-red-700`
  - [ ] 實作邏輯：
    ```javascript
    // 根據 API 回傳的 safety_level 動態設定卡片樣式
    const cardStyles = {
      green: 'bg-green-50 border-2 border-green-200 text-green-700',
      yellow: 'bg-yellow-50 border-2 border-yellow-200 text-yellow-700',
      red: 'bg-red-50 border-2 border-red-200 text-red-700'
    };
    ```

- [ ] **更新前端說明**
  - [ ] Line 404: 改成「實戰：極簡建議（1-2 句）」
  - [ ] Line 720: 改成「實戰：極簡建議（1-2 句）」
  - [ ] Line 405, 721: 改成「練習：完整分析（3-4 句）」

- [ ] **測試前端顯示**
  - [ ] Emergency 模式：顯示 1-2 個大卡片
  - [ ] Practice 模式：顯示 3-4 個大卡片
  - [ ] 確認大字顯示正常（text-xl md:text-2xl）
  - [ ] 確認燈號顏色正確（綠/黃/紅）

##### 1.5 測試

- [ ] **Integration Tests**
  - [ ] 測試 Emergency mode API
    - [ ] 回傳 1-2 句建議
    - [ ] 包含 safety_level
    - [ ] 建議從 200 句中選擇
  - [ ] 測試 Practice mode API
    - [ ] 回傳 3-4 句建議
    - [ ] 包含 safety_level
    - [ ] 建議從 200 句中選擇
  - [ ] 測試 GBQ 寫入
    - [ ] 資料成功寫入
    - [ ] 非同步執行不影響 API 回應
    - [ ] tenant_id 為 "island_parents"

- [ ] **手動測試**
  - [ ] 用 12/18 老師的實際對話測試
  - [ ] 驗證建議是否實用
  - [ ] 確認燈號判斷合理

#### Phase 2（本週內）

##### 2.1 後端優化

- [ ] **擴大分析範圍**
  - 從「最新 1 分鐘」→「最近 3-5 分鐘」
  - 保留完整對話上下文

- [ ] **增強 RAG 檢索**
  - top_k: 3 → 5-10
  - similarity_threshold: 0.5 → 0.3-0.4
  - 移除 200 字內容截斷

##### 2.2 前端智能化

- [ ] **動態調整分析頻率（根據燈號）**
  - [ ] 🟢 **綠燈**（對話安全）：60 秒觸發一次
    - 當前狀態良好，維持正常監控頻率
  - [ ] 🟡 **黃燈**（需要調整）：30 秒觸發一次
    - 對話出現警訊，提高監控密度
  - [ ] 🔴 **紅燈**（立刻修正）：15 秒觸發一次
    - 危險狀況，最高監控頻率
  - [ ] 實作邏輯：
    ```javascript
    // 根據上次分析的 safety_level 動態調整
    let analysisInterval = 60; // 預設
    if (lastSafetyLevel === 'red') analysisInterval = 15;
    else if (lastSafetyLevel === 'yellow') analysisInterval = 30;
    else analysisInterval = 60;

    // Line 2036: 改成動態判斷
    if (elapsed > 0 && elapsed % analysisInterval === 0) {
        analyzeTranscript();
    }
    ```
  - [ ] 前端顯示倒數計時（下次分析還有 X 秒）
  - [ ] 燈號變化時即時調整間隔

- [x] ~~**卡片去重邏輯（避免重複建議）**~~ → **已移除，不需要實作**

##### 2.3 測試

- [ ] **Integration Tests**
  - 測試 Practice mode 建議從 200 句中選擇
  - 測試燈號判斷正確性
  - 測試 GBQ 寫入
  - 測試動態分析頻率切換

---

## ✅ Check（檢查評估）

### 簡單評估指標

**系統效能**：
- 回應時間：< 2 秒（Practice mode）
- 建議來源：100% 從 200 句專家建議中選擇
- 燈號正確率：≥ 80%（vs 專家標註）

**使用者體驗**：
- 建議相關性：老師/家長反饋「有用」
- 對話品質改善：燈號變化趨勢（紅→橘→綠）
- 卡片去重：避免連續顯示相同建議

### 測試方法

1. **人工測試**（立即）
   - 用 12/18 的實際對話測試
   - 驗證建議是否符合情境

2. **Integration Tests**（本週）
   - 測試 API 回傳格式
   - 測試建議長度限制
   - 測試 DB 寫入

3. **使用者反饋**（持續）
   - 收集老師/家長的真實反饋
   - 調整建議句庫和 Prompt

---

## 🔄 Act（持續改進）

### 實作歷程

#### 2025-12-25

**問題診斷**：
- 發現 Realtime API 七大問題
- 分析範圍太窄是核心問題
- LLM 自己生成建議（應該從專家建議中選）

**專家輸入**：
- 整合 200 句專家建議（綠 70 / 橘 65 / 紅 65）
- 引入 Bridge 技巧（焦點解決取向）
- 重新定義回饋架構（連結 > 評價）

**決策**：
- ✅ 簡單優先：直接放 Prompt，不用向量搜尋
- ✅ GBQ 持久化：Web 版只寫 GBQ（tenant_id = "island_parents"）
- ✅ 移除字數限制：200 句都能用，不限制字數
- ❌ 拒絕過度工程：不做 Dashboard、複雜 ETL、A/B Testing

**Phase 1.1 完成**（2025-12-25）：
- ✅ 建立 `app/config/parenting_suggestions.py`
- ✅ 建立 `docs/parenting_expert_suggestions.md`
- ✅ 建立 `tests/unit/test_parenting_suggestions.py`（28 個測試）
- ✅ 驗證通過（200 句，無重複）
- ✅ 移除字數限制（所有建議都可用）
- ✅ **所有測試通過（28/28）**

### 下一步行動

**本週目標**：
1. ✅ 建立 200 句建議檔案（已完成 - Phase 1.1）
2. ✅ 修改 Emergency/Practice Mode Prompt（已完成 - Phase 1.2）
3. ✅ 修正交通號誌系統（orange → yellow）（已完成 - Phase 1.2）
4. ✅ 建立 GBQ Table 並實作非同步寫入（已完成 - Phase 1.3）
5. ⏳ 前端調整（卡片顏色、去重邏輯）（待開始 - Phase 1.4）

**成功標準**：
- ✅ 200 句建議檔案建立並驗證通過（28/28 unit tests passed）
- ✅ Emergency/Practice 建議從 200 句中選擇（9/9 integration tests passed）
- ✅ 包含 Bridge 技巧結構（已實作）
- ✅ 使用紅黃綠交通號誌系統（已修正）
- ✅ 分析結果存入 GBQ（tenant_id = "island_parents"）（5/5 GBQ tests passed）
- ⏳ 卡片顏色根據燈號變化（綠/黃/紅）
- ⏳ 避免重複顯示相同建議

---

## 附錄

### 當前 RAG 系統架構

**向量資料庫**：
- Chroma DB（本地向量儲存）
- Collection: `parents_knowledge_base`

**嵌入模型**：
- OpenAI `text-embedding-3-small`
- 維度: 1536

**檢索策略**：
- 相似度搜尋（Similarity Search）
- 預設檢索文件數: 5

**相關檔案**：
- `/app/services/rag_service.py` - RAG 核心邏輯
- `/app/api/realtime.py` - Realtime 分析 API
- `/app/models/session.py` - Session 資料模型
- `/tests/integration/test_rag_*.py` - RAG 測試

---

**Note**: 本文件為 living document，隨著系統演進持續更新。所有重大變更請同步更新 CHANGELOG.md。
