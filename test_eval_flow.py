...
def main(chunk_strategy=None):
    ...
    if needs_answer:
        ...
        for idx in needs_answer:
            ...
            if rag_response.ok:
                ...
                tc['contexts'] = tc.get('contexts') or [c['text'] for c in rag_result.get('citations', [])]
                print(f"   ✓ 答案已生成")
            else:
    ...
    print("\n🧪 步驟 4: 建立實驗...")
    exp_data = {
        "name": f"測試流程 - {testset['name']}" + (f" ({chunk_strategy})" if chunk_strategy else ""),
        "description": f"完整流程測試 - 使用測試集「{testset['name']}」" + (f"，策略: {chunk_strategy}" if chunk_strategy else ""),
    ...
    try:
        ...
        if response.ok:
            ...
            print(f"   - Context Precision: {result.get('avg_context_precision', 'N/A')}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--chunk-strategy', type=str, help='Chunk strategy to use (e.g., rec_400_80, rec_512_100)')
