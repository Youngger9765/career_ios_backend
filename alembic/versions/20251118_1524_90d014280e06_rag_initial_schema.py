...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_collection_items_doc_id'), 'collection_items', ['doc_id'], unique=False)
    ...
    op.create_foreign_key('fk_agents_active_version', 'agents', 'agent_versions', ['active_version_id'], ['id'])
    ...
    op.create_index(op.f('ix_chat_logs_agent_id'), 'chat_logs', ['agent_id'], unique=False)
    ...
    op.create_table(
        ...
        sa.Column('topic_coverage', postgresql.JSON(astext_type=sa.Text()), nullable=True),
        sa.Column('knowledge_depth', postgresql.JSON(astext_type=sa.Text()), nullable=True),
