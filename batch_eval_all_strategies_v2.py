...
def create_experiment(strategy: Dict[str, Any]) -> str:
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/api/rag/evaluation/experiments",
            json=exp_data,
            timeout=10
        )
...
def run_evaluation(experiment_id: str, test_cases: List[Dict[str, Any]]) -> bool:
    ...
    try:
        ...
        if response.ok:
            ...
            print(f"      - Answer Relevancy: {result.get('avg_answer_relevancy', 0):.3f}")
...
def get_experiment_results(experiment_id: str) -> Dict[str, Any]:
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/api/rag/evaluation/experiments/{experiment_id}",
            timeout=10
        )
...
def evaluate_strategy(strategy: Dict[str, Any]) -> Dict[str, Any]:
    """評估單一策略"""
    print(f"\n{'=' * 80}")
    print(f"📊 評估策略: {strategy['name']} (size={strategy['size']}, overlap={strategy['overlap']})")
    print(f"{'=' * 80}\n")
...
def print_comparison_table(results: List[Dict[str, Any]]):
    ...
    # 表頭
    print(f"{'Strategy':<15} {'Status':<10} {'Faithfulness':<13} {'Answer Rel.':<13} {'Queries':<8} {'Time(s)':<8}")
    print("-" * 80)
    ...
    for r in results:
        ...
        print(f"{status_icon} {strategy:<13} {status:<10} {faith:<13} {ans_rel:<13} {queries:<8} {time_str:<8}")
...
def recommend_best_strategy(results: List[Dict[str, Any]]):
    ...
    # 找出各項指標最佳的策略
    best_faith = max(successful_results, key=lambda x: x['metrics']['faithfulness'] or 0)
    best_ans_rel = max(successful_results, key=lambda x: x['metrics']['answer_relevancy'] or 0)
    ...
    print(f"\n📈 各項指標最佳:")
    print(f"   - Faithfulness: {best_faith['strategy']} ({best_faith['metrics']['faithfulness']:.3f})")
    print(f"   - Answer Relevancy: {best_ans_rel['strategy']} ({best_ans_rel['metrics']['answer_relevancy']:.3f})")
...
def generate_report(results: List[Dict[str, Any]]):
    ...
    with open(md_file, 'w', encoding='utf-8') as f:
        f.write(f"# Chunk Strategy 批次評估報告\n\n")
        ...
        f.write(f"## 結果總覽\n\n")
        f.write(f"| 策略 | 狀態 | Faithfulness | Answer Relevancy | 執行時間(s) |\n")
        f.write(f"|------|------|--------------|------------------|-------------|\n")

        for r in results:
            if r['status'] == 'success' and 'metrics' in r:
                m = r['metrics']
                f.write(f"| {r['strategy']} | ✅ | {m['faithfulness']:.3f} | {m['answer_relevancy']:.3f} | {r['elapsed_time']:.1f} |\n")
            else:
                f.write(f"| {r['strategy']} | ❌ | N/A | N/A | {r['elapsed_time']:.1f} |\n")

        f.write(f"\n## 推薦策略\n\n")
        if successful:
            best = max(successful, key=lambda x: (x['metrics']['faithfulness'] or 0) * 0.6 + (x['metrics']['answer_relevancy'] or 0) * 0.4)
...
def main():
    ...
    print(f"\n✅ API server 運行中")
    print(f"   測試 {len(STRATEGIES)} 種策略: {', '.join([s['name'] for s in STRATEGIES])}")
