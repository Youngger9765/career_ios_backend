#!/usr/bin/env python3
"""
直接展示新舊版報告品質對比（無需實際API）

使用模擬的「典型舊版報告」vs「新版改善後報告」
"""

from app.utils.report_validators import (
    validate_report_structure,
    validate_citations,
    calculate_quality_score
)
from app.utils.report_quality import generate_quality_summary, get_quality_grade


# 模擬舊版報告（staging）- 典型問題
OLD_REPORT = """
【一、案主基本資料】
姓名：小美（化名）
性別：女性
年齡：28歲
職業：軟體工程師

【二、主訴問題】
- 個案陳述：工作迷茫，不知道未來方向
- 諮詢師觀察：缺乏職涯目標

【五、多層次因素分析】
個人因素：案主28歲，碩士學歷。[1]
人際因素：與家人關係良好。
環境因素：在科技公司工作。

【七、諮詢師的專業判斷】
問題假設：案主可能處於生涯探索期。[2]
需要進一步釐清價值觀。

【八、諮商目標與介入策略】
諮商目標：找到職涯方向
介入技術：使用卡片排序 [3] 和生涯幻遊 [4]

【十、諮詢師自我反思】
本次晤談建立良好關係。
"""


# 新版報告（feature）- 改善後
NEW_REPORT = """
【一、案主基本資料】
姓名：小美（化名）
性別：女性
年齡：28歲
學歷：碩士
職業：軟體工程師
經濟狀況：穩定
家庭關係：與父母同住

【二、主訴問題】
- 個案陳述：最近工作感到迷茫，3個月前升遷失敗後開始懷疑自己的職涯方向
- 諮詢師觀察：案主在建立期遇到發展瓶頸，同時面臨價值觀探索任務

【三、問題發展脈絡】
出現時間：3個月前升遷失敗後
發展歷程：從最初的挫折感，逐漸演變為對整體職涯方向的質疑
影響範圍：工作動機下降，對未來感到困惑

【四、求助動機與期待】
引發因素：升遷失敗觸發自我懷疑
求助期待：希望找到適合的職涯方向，提升工作動機
動機強度：主動求助，展現積極改變意願

【五、多層次因素分析】
根據 Super 生涯發展理論 [1]，案主28歲正處於建立期（25-44歲），此階段的發展任務是在職場中立足與穩定。
案主目前遭遇的困境符合此階段常見的「試誤」歷程，需透過探索來確認適合的發展路徑。

從自我效能理論觀點 [2]，案主升遷失敗的經驗可能降低其職涯自我效能感。
Bandura 指出，成功經驗是建立自我效能的重要來源，因此案主需要透過小步驟的成功累積來重建信心。

個人因素：碩士學歷、技術能力佳，但對管理角色信心不足
人際因素：父母期待穩定，可能與案主自我探索需求產生衝突
環境因素：科技產業變化快速，職涯路徑多元但也容易迷茫

【六、個案優勢與資源】
心理優勢：主動求助意願強、自我覺察能力佳
社會資源：穩定工作、家庭支持系統完整
專業資源：碩士學歷、技術專長

【七、諮詢師的專業判斷】
基於生涯建構理論 [3]，我認為案主的問題核心在於「生涯敘事」尚未整合。
案主需要重新建構自己的生涯故事，釐清「我是誰」、「我想成為誰」的問題。

從動機理論角度 [4]，案主展現的低動機可能源於缺乏內在動機連結。
Deci & Ryan 的自我決定論指出，當工作無法滿足自主性、勝任感、關係性需求時，內在動機會降低。

問題假設：案主處於生涯轉換期，需透過價值觀澄清與自我探索來重新定位職涯方向。

【八、諮商目標與介入策略】
諮商目標：
1. 短期（3個月）：完成價值觀澄清，識別3-5個核心職涯價值
2. 中期（6個月）：探索符合價值觀的職涯選項，制定具體行動計畫

介入技術：
1. 卡片排序法 [5]：根據 Holland 職業興趣理論，此技術能有效協助案主釐清價值觀優先順序。
   選擇理由：案主目前對「想要什麼」感到困惑，卡片排序提供具體化的選擇框架，降低抽象思考負擔。

2. 生涯幻遊 [6]：透過想像未來生活樣貌，連結內在渴望與外在選擇。
   選擇理由：案主理性分析能力強，但可能壓抑情感需求，幻遊技術能幫助其接觸deeper desires。

介入步驟：先用卡片排序建立認知清晰度，再用生涯幻遊連結情感動機，最後整合兩者制定行動計畫。

【九、預期成效與評估】
短期指標（1-3個月）：
- 完成價值觀卡片排序，產出前5大價值清單
- 能具體描述理想工作的3個關鍵特徵

長期指標（6-12個月）：
- 工作動機量表分數提升20%
- 制定至少2個具體職涯行動方案

可能調整：若案主在探索過程中發現深層議題（如家庭期待衝突），需調整策略納入家庭諮商。

【十、諮詢師自我反思】
本次晤談成功建立信任關係，案主願意開放分享內在困惑。
優點：有效同理案主的挫折感，提供理論支持讓案主感受到被理解。
可改進：未來可更深入探索升遷失敗對案主自我認同的影響，這可能是更核心的議題。
"""


def analyze_report(report_text: str, title: str, theories_count: int = 6):
    """分析報告品質"""
    print("\n" + "=" * 70)
    print(f"📊 {title}")
    print("=" * 70)

    structure = validate_report_structure(report_text)
    citation = validate_citations(report_text)
    score = calculate_quality_score(structure, citation)

    print(f"\n📋 結構完整性: {structure['coverage']:.0f}%")
    if structure['missing_sections']:
        print(f"   ❌ 缺少 {len(structure['missing_sections'])} 個段落:")
        for section in structure['missing_sections']:
            print(f"      - {section}")
    else:
        print("   ✅ 所有段落完整")

    print(f"\n📚 理論引用:")
    print(f"   總引用數: {citation['total_citations']} 個")
    print(f"   核心段落完整: {'✅ 是' if citation['all_critical_sections_cited'] else '❌ 否'}")
    print(f"   有理由說明: {'✅ 是' if citation['has_rationale'] else '❌ 否'}")

    print(f"\n   段落詳情:")
    for section, detail in citation['section_details'].items():
        status = detail['status']
        count = detail['citation_count']
        print(f"   {status} {section}: {count} 個引用")

    print(f"\n🎯 品質分數: {score:.1f}/100 ({get_quality_grade(score)})")

    # 詳細計分
    print(f"\n   計分細節:")
    print(f"   - 結構 (40%): {structure['coverage'] * 0.4:.1f}")

    if citation['all_critical_sections_cited']:
        print(f"   - 引用覆蓋 (40%): 40.0")
    else:
        cited = sum(1 for d in citation['section_details'].values() if d['has_citations'])
        print(f"   - 引用覆蓋 (40%): {(cited / 3) * 40:.1f}")

    print(f"   - 理由說明 (10%): {10.0 if citation['has_rationale'] else 0.0}")
    print(f"   - 引用數量 (10%): {min(citation['total_citations'] / 7, 1.0) * 10:.1f}")

    return {
        "structure_coverage": structure['coverage'],
        "missing_sections": len(structure['missing_sections']),
        "citation_count": citation['total_citations'],
        "critical_cited": citation['all_critical_sections_cited'],
        "has_rationale": citation['has_rationale'],
        "score": score,
        "grade": get_quality_grade(score)
    }


def compare_metrics(old: dict, new: dict):
    """對比新舊版本"""
    print("\n\n" + "=" * 70)
    print("📈 改善對比分析")
    print("=" * 70)

    metrics = [
        ("結構完整性", "structure_coverage", "%", False),
        ("缺少段落", "missing_sections", "個", True),  # 越少越好
        ("理論引用數", "citation_count", "個", False),
        ("核心段落引用", "critical_cited", "", False),
        ("理由說明", "has_rationale", "", False),
        ("品質分數", "score", "分", False),
    ]

    improvements = []

    for label, key, unit, reverse in metrics:
        old_val = old[key]
        new_val = new[key]

        # 格式化顯示
        if isinstance(old_val, bool):
            old_str = "✅" if old_val else "❌"
            new_str = "✅" if new_val else "❌"
            diff_str = ""
        elif isinstance(old_val, float):
            old_str = f"{old_val:.1f}{unit}"
            new_str = f"{new_val:.1f}{unit}"
            diff = new_val - old_val
            diff_str = f"({diff:+.1f})"
        else:
            old_str = f"{old_val}{unit}"
            new_str = f"{new_val}{unit}"
            diff = new_val - old_val
            diff_str = f"({diff:+d})"

        # 判斷是否改善
        if isinstance(old_val, bool):
            improved = new_val and not old_val
        elif reverse:
            improved = new_val < old_val
        else:
            improved = new_val > old_val

        status = "✅" if improved else ("➡️" if new_val == old_val else "❌")

        print(f"\n{status} {label}:")
        print(f"   舊版: {old_str}")
        print(f"   新版: {new_str} {diff_str}")

        if improved:
            if isinstance(old_val, float):
                if key == "score":
                    improvements.append(f"{label}提升 {new_val - old_val:.1f} 分")
                else:
                    improvements.append(f"{label}從 {old_val:.0f}% 提升到 {new_val:.0f}%")
            elif isinstance(old_val, bool):
                improvements.append(f"新增{label}")
            else:
                if reverse:
                    improvements.append(f"{label}減少 {old_val - new_val} {unit}")
                else:
                    improvements.append(f"{label}增加 {new_val - old_val} {unit}")

    print("\n" + "=" * 70)
    print("📊 總結")
    print("=" * 70)

    print(f"\n等級變化: {old['grade']} ➜ {new['grade']}")

    if old['score'] > 0:
        improvement_pct = ((new['score'] - old['score']) / old['score']) * 100
        print(f"整體改善: {improvement_pct:+.1f}%")

    if improvements:
        print(f"\n✅ 關鍵改善 ({len(improvements)} 項):")
        for i, imp in enumerate(improvements, 1):
            print(f"   {i}. {imp}")

    # 量化證明
    print("\n" + "=" * 70)
    print("🎯 量化證明")
    print("=" * 70)
    print(f"\n舊版問題:")
    if old['missing_sections'] > 0:
        print(f"  • 結構不完整，缺少 {old['missing_sections']} 個段落")
    if not old['critical_cited']:
        print(f"  • 核心段落引用不足")
    if not old['has_rationale']:
        print(f"  • 缺乏理論應用說明，只有引用編號")
    if old['citation_count'] < 5:
        print(f"  • 引用數量過少（{old['citation_count']} < 5）")

    print(f"\n新版改善:")
    if new['structure_coverage'] == 100:
        print(f"  ✅ 結構 100% 完整（10/10 段落）")
    if new['critical_cited']:
        print(f"  ✅ 核心段落 100% 引用")
    if new['has_rationale']:
        print(f"  ✅ 所有引用都有理論說明")
    if new['citation_count'] >= 6:
        print(f"  ✅ 引用數量充足（{new['citation_count']} ≥ 6）")


def main():
    print("\n" + "=" * 70)
    print("🧪 報告品質改善驗證測試")
    print("   （使用模擬報告直接對比，無需實際 API）")
    print("=" * 70)

    # 分析舊版
    old_metrics = analyze_report(OLD_REPORT, "舊版報告分析（staging - 典型問題）")

    # 分析新版
    new_metrics = analyze_report(NEW_REPORT, "新版報告分析（feature - 改善後）")

    # 對比
    compare_metrics(old_metrics, new_metrics)

    print("\n" + "=" * 70)
    print("✅ 測試完成！")
    print("=" * 70)


if __name__ == "__main__":
    main()
