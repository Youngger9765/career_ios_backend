#!/usr/bin/env python3
"""
測試策略 B：模擬只 cache system instruction（只發送新對話）

透過 Staging API 測試，這個策略假設：
- System instruction 已被 cache（這部分由 API server 處理）
- 只發送最新的對話片段
- 讓 Gemini 的 implicit caching 處理

實際場景：
- 第 1 分鐘：只發送 "對話1"
- 第 2 分鐘：只發送 "對話2" (不包含對話1)
- 第 3 分鐘：只發送 "對話3" (不包含對話1-2)

注意：這個策略可能導致 AI 缺少上下文，僅用於技術測試。
實際應用中不建議使用（會失去對話連貫性）。
"""
...
async def analyze_incremental(minute: int):
    """
    只發送當前分鐘的對話（不包含之前的）
    模擬「固定 cache」：system instruction 已被 cache，只發送新內容
    """
    print(f"\n{'='*80}")
    print(f"📍 第 {minute} 分鐘 - 策略 B（只有當前分鐘對話）")
    print(f"{'='*80}")

    # 只取當前分鐘
    current_dialog = CONVERSATION[minute]
    transcript_chars = len(current_dialog)
    estimated_tokens = int(transcript_chars * 0.4)  # 繁體中文約 0.4 tokens/char

    print(f"當前對話: 第 {minute} 分鐘")
    print(f"字符數: {transcript_chars}")
    print(f"估算 tokens: {estimated_tokens}")

    # 發送請求
    request_data = {
        "transcript": current_dialog,
        "speakers": [],
        "time_range": f"{minute-1}:00-{minute}:00",
    }
...
async def main():
    print("=" * 80)
    print("🧪 測試策略 B：只發送當前對話（模擬固定 cache）")
    print("=" * 80)
    print(f"測試時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"API: {REALTIME_ENDPOINT}")
    print("=" * 80)
    print("\n⚠️  警告：此策略會失去對話上下文，僅用於技術測試")
    print("=" * 80)
    ...
    for minute in range(1, 11):
        result = await analyze_incremental(minute)
    ...
    if successful:
        ...
        # 儲存結果
        output_file = "strategy_b_results.json"
        with open(output_file, "w", encoding="utf-8") as f:
            json.dump(
                {
                    "timestamp": datetime.now().isoformat(),
                    "strategy": "B - Incremental Only (Simulated Fixed Cache)",
                    "description": "只發送當前對話，模擬固定 cache system instruction",
                    "warning": "此策略會失去對話上下文，僅用於技術測試",
                    "results": results,
                    "summary": {
                        ...
                        "cache_creation_time": 0,  # N/A for implicit caching
                    },
                },
                f,
                ensure_ascii=False,
